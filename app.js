...
// F:/MOPrograms/rent_xiaomei_uniapp
// F:/MOPrograms/flutter_demo
getProgramsMd("F:/MOPrograms/flutter_demo", "flutter")
function getProgramsMd(path, type) {
    let mdContent = ""
    createProgramsMd(path)
    fs.writeFileSync(`${path}/项目目录结构_test.md`, mdContent)
    //创建目录结构MD文件，localPath,路径
    function createProgramsMd(localPath, num) {
        if (!num) {
            num = 1
        }
...
function checkDirectoryName(name, type) {
    let checkArr = [
        {
            type: "uniapp",
...
function changeFontColor(content, type) {
    let changeColorArr = [
        {
            type: "uniapp",
            funs: (content) => {
